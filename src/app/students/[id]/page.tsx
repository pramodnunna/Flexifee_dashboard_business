            ...
            <table>
              ...
              <tbody>
                {student.transactions.map(t => {
                  const comm = t.commissionAmount || t.commissionPaid || 0;
                  const net = t.revenueEarned - comm + t.bankCommission;
                  return (
                    <tr key={t.id}>
                      ...
                      <td style={{ color: "var(--destructive)" }}>-₹{comm.toLocaleString('en-IN')}</td>
                      <td style={{ color: "var(--secondary)" }}>+₹{t.bankCommission.toLocaleString('en-IN')}</td>
                      <td style={{ fontWeight: 600, color: net >= 0 ? 'var(--primary)' : 'var(--destructive)' }}>₹{net.toLocaleString('en-IN')}</td>
                    </tr>
                  );
                })}
              </tbody>
